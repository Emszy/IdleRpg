
import playerCoffeeHurt from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/hurt.png"
import playerCoffeeIdle from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/idle.png"
import playerCoffeeMagic from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/magic.png"
import playerCoffeeShoot from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/shoot.png"
import playerCoffeeSwing from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/swing.png"
import playerCoffeeThrust from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/thrust.png"
import playerCoffeeWalk from "../../assets/images/spriteSheets/Player/partSheets/bases/Coffee/walk.png"


import blackShirtHurt from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/hurt.png"
import blackShirtIdle from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/idle.png"
import blackShirtMagic from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/magic.png"
import blackShirtShoot from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/shoot.png"
import blackShirtSwing from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/swing.png"
import blackShirtThrust from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/thrust.png"
import blackShirtWalk from "../../assets/images/spriteSheets/Player/partSheets/shirts/Black/walk.png"

import blackPantsHurt from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/hurt.png"
import blackPantsIdle from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/idle.png"
import blackPantsMagic from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/magic.png"
import blackPantsShoot from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/shoot.png"
import blackPantsSwing from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/swing.png"
import blackPantsThrust from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/thrust.png"
import blackPantsWalk from "../../assets/images/spriteSheets/Player/partSheets/pants/Black/walk.png"

import blackMessyHair from "../../assets/images/spriteSheets/Player/fullSheet/Clothes01/HalfMessy_black.png"

import bronzeAxe from "../../assets/images/spriteSheets/Player/fullSheet/axes/bronze.png"
import ironAxe from "../../assets/images/spriteSheets/Player/fullSheet/axes/iron.png"
import goldAxe from "../../assets/images/spriteSheets/Player/fullSheet/axes/gold.png"
import platinumAxe from "../../assets/images/spriteSheets/Player/fullSheet/axes/platinum.png"
import diamondAxe from "../../assets/images/spriteSheets/Player/fullSheet/axes/diamond.png"

import bronzePickaxe from "../../assets/images/spriteSheets/Player/fullSheet/pickaxes/bronze.png"
import ironPickaxe from "../../assets/images/spriteSheets/Player/fullSheet/pickaxes/iron.png"
import goldPickaxe from "../../assets/images/spriteSheets/Player/fullSheet/pickaxes/gold.png"
import platinumPickaxe from "../../assets/images/spriteSheets/Player/fullSheet/pickaxes/platinum.png"
import diamondPickaxe from "../../assets/images/spriteSheets/Player/fullSheet/pickaxes/diamond.png"

import bronzeSword from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/slash/bronze.png"
import ironSword from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/slash/iron.png"
import goldSword from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/slash/gold.png"
import platinumSword from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/slash/platinum.png"
import diamondSword from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/slash/diamond.png"


import oakBow from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/shoot/oak.png"
import mapleBow from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/shoot/maple.png"
import mahogonyBow from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/shoot/mahogony.png"
import magicBow from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/shoot/magic.png"
import superBow from "../../assets/images/spriteSheets/Player/fullSheet/weapons/male/shoot/super.png"


import bronzeShield from "../../assets/images/spriteSheets/Player/fullSheet/shields/bronze.png"
import ironShield from "../../assets/images/spriteSheets/Player/fullSheet/shields/iron.png"
import goldShield from "../../assets/images/spriteSheets/Player/fullSheet/shields/gold.png"
import platinumShield from "../../assets/images/spriteSheets/Player/fullSheet/shields/platinum.png"
import diamondShield from "../../assets/images/spriteSheets/Player/fullSheet/shields/diamond.png"

import bronzeHelm from "../../assets/images/spriteSheets/Player/fullSheet/armor/helm/bronzeHelm.png"
import ironHelm from "../../assets/images/spriteSheets/Player/fullSheet/armor/helm/ironHelm.png"
import goldHelm from "../../assets/images/spriteSheets/Player/fullSheet/armor/helm/goldHelm.png"
import platinumHelm from "../../assets/images/spriteSheets/Player/fullSheet/armor/helm/platinumHelm.png"
import diamondHelm from "../../assets/images/spriteSheets/Player/fullSheet/armor/helm/diamondHelm.png"


import bronzeChest from "../../assets/images/spriteSheets/Player/fullSheet/armor/chest/bronzeChest.png"
import ironChest from "../../assets/images/spriteSheets/Player/fullSheet/armor/chest/ironChest.png"
import goldChest from "../../assets/images/spriteSheets/Player/fullSheet/armor/chest/goldChest.png"
import platinumChest from "../../assets/images/spriteSheets/Player/fullSheet/armor/chest/platinumChest.png"
import diamondChest from "../../assets/images/spriteSheets/Player/fullSheet/armor/chest/diamondChest.png"

//start of enemies 

import lightDrake from "../../assets/images/spriteSheets/enemies/Reptile/Drake_male.png"
import lightDrakeTail from "../../assets/images/spriteSheets/enemies/Reptile/TailBack_male.png"
import lightDrakeWings from "../../assets/images/spriteSheets/enemies/Reptile/WingBack_male.png"

import blueDrake from "../../assets/images/spriteSheets/enemies/Reptile/Drake_male_blue.png"
import blueDrakeTail from "../../assets/images/spriteSheets/enemies/Reptile/TailBack_male_blue.png"
import blueDrakeTailFront from "../../assets/images/spriteSheets/enemies/Reptile/TailFront_male_blue.png"

import blueDrakeWings from "../../assets/images/spriteSheets/enemies/Reptile/WingBack_male_blue.png"
import blueDrakeWingsFront from "../../assets/images/spriteSheets/enemies/Reptile/WingFront_male_blue.png"

import skeleton from "../../assets/images/spriteSheets/enemies/skeleton.png"
import maleor from "../../assets/images/spriteSheets/enemies/maleorcfullsheet.png"


import blueImpWalk from "../../assets/images/spriteSheets/enemies/imp/blue/walk.png"
import blueImpAttack from "../../assets/images/spriteSheets/enemies/imp/blue/attack.png"
import blueImpDeath from "../../assets/images/spriteSheets/enemies/imp/blue/death.png"



import treesSprite from "../../assets/images/spriteSheets/resources/treeSheet.png"
import oreImages from "../../assets/images/spriteSheets/resources/oreResource.png"


import chickenWalk from "../../assets/images/spriteSheets/npc/chicken_walk.png"
import cowWalk from "../../assets/images/spriteSheets/npc/cow_walk.png"
import llamaWalk from "../../assets/images/spriteSheets/npc/llama_walk.png"
import turkeyWalk from "../../assets/images/spriteSheets/npc/turkey.png"

import chickenEat from "../../assets/images/spriteSheets/npc/chicken_eat.png"
import pigEat from "../../assets/images/spriteSheets/npc/pig_eat.png"


import farmSoil from "../../assets/images/spriteSheets/terrain/plowed_soil.png"

import plantImages from "../../assets/images/spriteSheets/resources/plants.png"
import cookedPlantImages from "../../assets/images/spriteSheets/resources/cookedPlants.png"

import mapAtlasImage from "../../assets/images/spriteSheets/terrain/terrain_atlas.png"


import homeAtlasOne from "../../assets/images/spriteSheets/terrain/build_atlas.png"
import homeAtlasTwo from "../../assets/images/spriteSheets/terrain/obj_misk_atlas.png"
import wellImage from "../../assets/images/spriteSheets/terrain/lpc-well.png"


//start of items

import oreItems from "../../assets/images/spriteSheets/resources/ores.png"
import barItems from "../../assets/images/spriteSheets/resources/barItems.png"
import woodItems from "../../assets/images/spriteSheets/resources/woodItems.png"
import seeds from "../../assets/images/spriteSheets/resources/seeds.png"
import potionImages from "../../assets/images/spriteSheets/resources/potions.png"

import cookedFood from "../../assets/images/spriteSheets/resources/food.png"


import magicStones from "../../assets/images/spriteSheets/Player/partSheets/magic/magicStones.png"

// start of UI


import inventoryUI from "../../assets/images/spriteSheets/UI/inventoryUI2.png"
import armorUI from "../../assets/images/spriteSheets/UI/armorUI.png"
import statsUI from "../../assets/images/spriteSheets/UI/statsUI.png"
import redButton from "../../assets/images/spriteSheets/UI/Buttons/Button19.png"
import greenButton from "../../assets/images/spriteSheets/UI/Buttons/Button18.png"
import aquaButton from "../../assets/images/spriteSheets/UI/Buttons/Button5.png"
import blueButton from "../../assets/images/spriteSheets/UI/Buttons/Button20s.png"

import upArrow from "../../assets/images/spriteSheets/UI/upArrow.png"
import downArrow from "../../assets/images/spriteSheets/UI/downArrow.png"
import closeButton from "../../assets/images/spriteSheets/UI/closeButton.png"
import bankScrollBar from "../../assets/images/spriteSheets/UI/bankScrollBar.png"


import icons from "../../assets/images/spriteSheets/UI/icons.png"
import bankBackGround from "../../assets/images/spriteSheets/UI/bankBackGround.png"
import itemBorder from "../../assets/images/spriteSheets/UI/itemBorder.png"

import itemInfoBackGround from "../../assets/images/spriteSheets/UI/itemInfoBackGround.png"


import actionButtonBackGround from "../../assets/images/spriteSheets/UI/actionButtonBackGround.png"

import healthBar from "../../assets/images/spriteSheets/UI/healthBars/healthBar.png"
import thirstBar from "../../assets/images/spriteSheets/UI/healthBars/thirstBar.png"
import hungerBar from "../../assets/images/spriteSheets/UI/healthBars/foodBar.png"
import emptyBar from "../../assets/images/spriteSheets/UI/healthBars/emptyBar.png"
 

function npcImage(srcs) {
	function directions(end) {
		return ({
					up : {
					index : 0,
					end : end,
					},
					left : {
						index : 1,
						end : end,
					},
					down : {
						index : 2,
						end : end,
					},
					right : {
						index : 3,
						end : end,
					}
				})
	}

	let imgObject = {
		eat:  {
			img : new Image(),
			directions : directions(3)
		},
		walk : {
			img : new Image(),
			directions : directions(3)
		},
	}

	imgObject.walk.img.src = srcs.walk
	imgObject.eat.img.src = srcs.eat
	return (imgObject)
}


function partSheetImage(srcs) {

	function directions(end) {
		return ({
					up : {
					index : 0,
					end : end,
					},
					left : {
						index : 1,
						end : end,
					},
					down : {
						index : 2,
						end : end,
					},
					right : {
						index : 3,
						end : end,
					}
				})
	}

	let imgObject = {
		hurt:  {
			img : new Image(),
			directions : directions(5)
		},
		idle: {
			img : new Image(),
			directions : directions(0)
		},
		magic: {
			img : new Image(),
			directions : directions(6)
		},
		shoot: {
			img : new Image(),
			directions : directions(12)

		},
		swing: {
			img : new Image(),
			directions : directions(5)
		},
		thrust: {
			img : new Image(),
			directions : directions(7)
		},
		walk : {
			img : new Image(),
			directions : directions(7)

		},
	}

	imgObject.hurt.img.src = srcs.hurt
	imgObject.idle.img.src = srcs.idle
	imgObject.magic.img.src = srcs.magic
	imgObject.shoot.img.src = srcs.shoot
	imgObject.swing.img.src = srcs.swing
	imgObject.thrust.img.src = srcs.thrust
	imgObject.walk.img.src = srcs.walk
	return (imgObject)
}


function fullSheetImage(src) {

	function directions(upIndex, leftIndex, downIndex, rightIndex, end) {
		return ({
					up : {
						index : upIndex,
						end : end,
					},
					left : {
						index : leftIndex,
						end : end,
					},
					down : {
						index : downIndex,
						end : end,
					},
					right : {
						index : rightIndex,
						end : end,
					}
				})
	}

	let imgObject = {

		hurt:  {
			img : new Image(),
			directions : directions(20,20, 20, 20, 5)
		},

		idle: {
			img : new Image(),
			directions : directions(8, 9, 10, 11, 0)
		},

		magic: {
			img : new Image(),
			directions : directions(0, 1, 2, 3, 6)
		},

		shoot: {
			img : new Image(),
			directions : directions(16, 17, 18, 19, 12)

		},

		swing: {
			img : new Image(),
			directions : directions(12, 13, 14, 15, 5)
		},

		thrust: {
			img : new Image(),
			directions : directions(4, 5, 6, 7, 7)
		},

		walk : {
			img : new Image(),
			directions : directions(8, 9, 10, 11, 7)
		},
	}

	imgObject.hurt.img.src = src
	imgObject.idle.img.src = src
	imgObject.magic.img.src = src
	imgObject.shoot.img.src = src
	imgObject.swing.img.src = src
	imgObject.thrust.img.src = src
	imgObject.walk.img.src = src
	return (imgObject)
}


function singleImage(img) {
	let image = new Image();
	image.src = img;

	return image;
}


let playerImages = {
	coffee : {
		size : 64,
		images : partSheetImage({
						hurt : playerCoffeeHurt,
						idle : playerCoffeeIdle,
						magic : playerCoffeeMagic,
						shoot : playerCoffeeShoot,
						swing : playerCoffeeSwing,
						thrust : playerCoffeeThrust,
						walk : playerCoffeeWalk,
				})
	}
}


let enemyImages = {
	lightDrake : {
		size : 64,
		images: fullSheetImage(lightDrake)
	},
	lightDrakeTail : {
		size : 64,
		images: fullSheetImage(lightDrakeTail)
	},
	lightDrakeWings : {
		size : 64,
		images: fullSheetImage(lightDrakeWings)
	},

	blueDrake : {
		size : 64,
		images: fullSheetImage(blueDrake)
	},
	blueDrakeTail : {
		size : 64,
		images: fullSheetImage(blueDrakeTail)
	},
	blueDrakeWings : {
		size : 64,
		images: fullSheetImage(blueDrakeWings)
	},
	blueDrakeWingsFront : {
		size : 64,
		images: fullSheetImage(blueDrakeWingsFront)
	},
	blueDrakeTailFront : {
		size : 64,
		images: fullSheetImage(blueDrakeTailFront)
	},
	skeleton : {
		size : 64,
		images: fullSheetImage(skeleton)
	},

	maleor : {
		size : 64,
		images: fullSheetImage(maleor)
	},

	blueImp : {
		size : 64,
		images : partSheetImage({
						hurt : blueImpDeath,
						idle : blueImpWalk,
						magic : blueImpAttack,
						shoot : blueImpAttack,
						swing : blueImpAttack,
						thrust : blueImpAttack,
						walk : blueImpWalk,
				})
	},


}


let playerShirts = {
	black: {
		size : 64,
		images : partSheetImage({
						hurt : blackShirtHurt,
						idle : blackShirtIdle,
						magic : blackShirtMagic,
						shoot : blackShirtShoot,
						swing : blackShirtSwing,
						thrust : blackShirtThrust,
						walk : blackShirtWalk,
				})
	}
}


let playerPants = {
	black: {
		size : 64,
		images : partSheetImage({
						hurt : blackPantsHurt,
						idle : blackPantsIdle,
						magic : blackPantsMagic,
						shoot : blackPantsShoot,
						swing : blackPantsSwing,
						thrust : blackPantsThrust,
						walk : blackPantsWalk,
				})
	}
}

let playerHair = {
	black: {
		size : 64,
		images : fullSheetImage(blackMessyHair)
	}
}

let playerTools = {
	bronzeAxe: {
		img : singleImage(bronzeAxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(bronzeAxe)
	},

	ironAxe: {
		img : singleImage(ironAxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(ironAxe)
	},

	goldAxe: {
		img : singleImage(goldAxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(goldAxe)
	},

	platinumAxe: {
		img : singleImage(platinumAxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(platinumAxe)
	},

	diamondAxe: {
		img : singleImage(diamondAxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(diamondAxe)
	},









	bronzePickaxe: {
		img : singleImage(bronzePickaxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(bronzePickaxe)
	},

	ironPickaxe: {
		img : singleImage(ironPickaxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(ironPickaxe)
	},

	goldPickaxe: {
		img : singleImage(goldPickaxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(goldPickaxe)
	},

	platinumPickaxe: {
		img : singleImage(platinumPickaxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(platinumPickaxe)
	},

	diamondPickaxe: {
		img : singleImage(diamondPickaxe),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(diamondPickaxe)
	},









}


let playerWeapons = {
	bronzeSword: {
		size : 64,
		img : singleImage(bronzeSword),
		pos : mapTile(0, 128, 64, 64),
		images : partSheetImage({
						hurt : bronzeSword,
						idle : bronzeSword,
						magic : bronzeSword,
						shoot : bronzeSword,
						swing : bronzeSword,
						thrust : bronzeSword,
						walk : bronzeSword,
				})
	},

	ironSword: {
		size : 64,
		img : singleImage(ironSword),
		pos : mapTile(0, 128, 64, 64),
		images : partSheetImage({
						hurt : ironSword,
						idle : ironSword,
						magic : ironSword,
						shoot : ironSword,
						swing : ironSword,
						thrust : ironSword,
						walk : ironSword,
				})
	},

	goldSword: {
		size : 64,
		img : singleImage(goldSword),
		pos : mapTile(0, 128, 64, 64),
		images : partSheetImage({
						hurt : goldSword,
						idle : goldSword,
						magic : goldSword,
						shoot : goldSword,
						swing : goldSword,
						thrust : goldSword,
						walk : goldSword,
				})
	},

	platinumSword: {
		size : 64,
		img : singleImage(platinumSword),
		pos : mapTile(0, 128, 64, 64),
		images : partSheetImage({
						hurt : platinumSword,
						idle : platinumSword,
						magic : platinumSword,
						shoot : platinumSword,
						swing : platinumSword,
						thrust : platinumSword,
						walk : platinumSword,
				})
	},

	diamondSword: {
		size : 64,
		img : singleImage(diamondSword),
		pos : mapTile(0, 128, 64, 64),
		images : partSheetImage({
						hurt : diamondSword,
						idle : diamondSword,
						magic : diamondSword,
						shoot : diamondSword,
						swing : diamondSword,
						thrust : diamondSword,
						walk : diamondSword,
				})
	},





	oakBow: {
		size : 64,
		img : singleImage(oakBow),
		pos : mapTile(128, 64, 64, 64),
		images : partSheetImage({
						hurt : oakBow,
						idle : oakBow,
						magic : oakBow,
						shoot : oakBow,
						swing : oakBow,
						thrust : oakBow,
						walk : oakBow,
				})
	},

	mapleBow: {
		size : 64,
		img : singleImage(mapleBow),
		pos : mapTile(128, 64, 64, 64),
		images : partSheetImage({
						hurt : mapleBow,
						idle : mapleBow,
						magic : mapleBow,
						shoot : mapleBow,
						swing : mapleBow,
						thrust : mapleBow,
						walk : mapleBow,
				})
	},
	mahogonyBow: {
		size : 64,
		img : singleImage(mahogonyBow),
		pos : mapTile(128, 64, 64, 64),
		images : partSheetImage({
						hurt : mahogonyBow,
						idle : mahogonyBow,
						magic : mahogonyBow,
						shoot : mahogonyBow,
						swing : mahogonyBow,
						thrust : mahogonyBow,
						walk : mahogonyBow,
				})
	},
	magicBow: {
		size : 64,
		img : singleImage(magicBow),
		pos : mapTile(128, 64, 64, 64),
		images : partSheetImage({
						hurt : magicBow,
						idle : magicBow,
						magic : magicBow,
						shoot : magicBow,
						swing : magicBow,
						thrust : magicBow,
						walk : magicBow,
				})
	},
	superBow: {
		size : 64,
		img : singleImage(superBow),
		pos : mapTile(128, 64, 64, 64),
		images : partSheetImage({
						hurt : superBow,
						idle : superBow,
						magic : superBow,
						shoot : superBow,
						swing : superBow,
						thrust : superBow,
						walk : superBow,
				})
	}
}

let playerArmor = {

	bronzeShield: {
		img : singleImage(bronzeShield),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(bronzeShield)
	},

	ironShield: {
		img : singleImage(ironShield),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(ironShield)
	},

	goldShield: {
		img : singleImage(goldShield),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(goldShield)
	},

	platinumShield: {
		img : singleImage(platinumShield),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(platinumShield)
	},
	diamondShield: {
		img : singleImage(diamondShield),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(diamondShield)
	},

	bronzeHelm: {
		img : singleImage(bronzeHelm),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(bronzeHelm)
	},

	ironHelm: {
		img : singleImage(ironHelm),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(ironHelm)
	},

	goldHelm: {
		img : singleImage(goldHelm),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(goldHelm)
	},

	platinumHelm: {
		img : singleImage(platinumHelm),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(platinumHelm)
	},

	diamondHelm: {
		img : singleImage(diamondHelm),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(diamondHelm)
	},

	bronzeChest: {
		img : singleImage(bronzeChest),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(bronzeChest)
	},

	ironChest: {
		img : singleImage(ironChest),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(ironChest)
	},

	goldChest: {
		img : singleImage(goldChest),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(goldChest)
	},

	platinumChest: {
		img : singleImage(platinumChest),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(platinumChest)
	},

	diamondChest: {
		img : singleImage(diamondChest),
		pos : mapTile(0, 640, 64, 64),
		size : 64,
		images: fullSheetImage(diamondChest)
	},
}

let trees = {
	oak : {
		img : singleImage(treesSprite),
		pos : {
			x : 0,
			y : 0,
			width : 96,
			height : 140,
		},
	},
	maple : {
		img : singleImage(treesSprite),
		pos : {
			x : 96,
			y : 0,
			width : 96,
			height : 140,
		},
	},

	mahogony : {
		img : singleImage(treesSprite),
		pos : {
			x : 192,
			y : 0,
			width : 96,
			height : 140,
		},
	},
	magic : {
		img : singleImage(treesSprite),
		pos : {
			x : 288,
			y : 0,
			width : 96,
			height : 140,
		},
	},
	super : {
		img : singleImage(treesSprite),
		pos : {
			x : 0,
			y : 140,
			width : 96,
			height : 140,
		},
	}
}

let ore = {
	tin : {
		img : singleImage(oreImages),
		size : 64,
		pos : {
			x : 0,
			y : 64,
			width : 64,
			height : 64,
		},
	},

	copper : {
		img : singleImage(oreImages),
		size : 64,
		pos : {
			x : 64,
			y : 64,
			width : 64,
			height : 64,
		},
	},
	iron : {
		img : singleImage(oreImages),
		size : 64,
		pos : {
			x : 128,
			y : 64,
			width : 64,
			height : 64,
		},
	},
	gold : {
		img : singleImage(oreImages),
		size : 64,
		pos : {
			x : 0,
			y : 128,
			width : 64,
			height : 64,
		},
	},
	platinum : {
		img : singleImage(oreImages),
		size : 64,
		pos : {
			x : 64,
			y : 128,
			width : 64,
			height : 64,
		},
	},
	diamond : {
		img : singleImage(oreImages),
		size : 64,
		pos : {
			x : 128,
			y : 128,
			width : 64,
			height : 64,
		},
	}
}

let npc = {
	chicken : {
		images : npcImage({walk: chickenWalk, eat: chickenEat}),
		size : 32,
		
	},

	cow : {
		images : npcImage({walk: cowWalk, eat: cowWalk}),
		size : 128,
		
	},

	llama : {
		images : npcImage({walk: llamaWalk, eat: llamaWalk}),
		size : 128,
		
	},
	pig : {
		images : npcImage({walk: pigEat, eat: pigEat}),
		size : 128,
		
	},

	turkey : {
		images : npcImage({walk: turkeyWalk, eat: turkeyWalk}),
		size : 128,
	},

}

let farmImages = {
	plot : {
		img: singleImage(farmSoil),
		
		leftTop : {
			width : 32,
			height : 32,
			x : 0,
			y : 64,
		},

		leftMiddle : {
			width : 32,
			height : 32,
			x : 0,
			y : 96,
		},

		leftBottom : {
			width : 32,
			height : 32,
			x : 0,
			y : 128,
		},
		bottomMiddle : {
			width : 32,
			height : 32,
			x : 32,
			y : 128,
		},
		bottomRight: {
			width : 32,
			height : 32,
			x : 64,
			y : 128,
		},

		middleTop : {
			width : 32,
			height : 32,
			x : 32,
			y : 64,
		},

		rightTop : {
			width : 32,
			height : 32,
			x : 64,
			y : 64,
		},

		rightMiddle : {
			width : 32,
			height : 32,
			x : 64,
			y : 96,
		},

		middle : {
			width : 32,
			height : 32,
			x : 32,
			y : 96,
		},

	}
}

function makeFarmPlant(x,y) {
	return ({
			img : singleImage(plantImages),
			width : 32,
			height: 64,
			end : 4,
			pos : {
				x: x,
				y: y,
			}
		})
}

let farmPlants = {
	tomato: makeFarmPlant(0, 0),
	potatoe: makeFarmPlant(32, 0),
	carrot: makeFarmPlant(64, 0),
	artichoke: makeFarmPlant(96, 0),
	cucumber: makeFarmPlant(160, 0),
	corn : makeFarmPlant(192, 0),
}


function mapTile(x,y,width,height) {
	return ({
		x:x,
		y:y,
		width : width,
		height: height,
	})
}

let mapAtlas = {
	img : singleImage(mapAtlasImage),
	terrain : {
		dirt : {
			light : {
				base : {
					hole : mapTile(480, 160, 32, 32),
					specks : mapTile(512, 160, 32, 32),
					main : mapTile(544, 160, 32, 32),
				},
				patch : mapTile(480, 64, 96, 96),
				bigHole: mapTile(512, 0, 64, 64),
				smallSpot: mapTile(480, 0, 32, 32),
				tinySpot: mapTile(480, 32, 32, 32),
			},

			dark : {
				base : {
					hole : mapTile(576, 160, 32, 32),
					specks : mapTile(608, 160, 32, 32),
					main : mapTile(640, 160, 32, 32),
				},
				patch : mapTile(576, 64, 96, 96),
				bigHole: mapTile(608, 0, 64, 64),
				smallSpot: mapTile(576, 0, 32, 32),
				tinySpot: mapTile(576, 32, 32, 32),
			},
		},

		grass : {
			light : {
				base : {
					hole : mapTile(672, 160, 32, 32),
					specks : mapTile(704, 160, 32, 32),
					main : mapTile(736, 160, 32, 32),
				},
				patch : mapTile(672, 64, 96, 96),
				bigHole: mapTile(704, 0, 64, 64),
				smallSpot: mapTile(672, 0, 32, 32),
				tinySpot: mapTile(672, 32, 32, 32),
			},
		},

		lava : {
			light : {
				base : {
					hole : mapTile(480, 160 + 192, 32, 32),
					specks : mapTile(512, 160 + 192, 32, 32),
					main : mapTile(544, 160 + 192, 32, 32),
				},
				patch : mapTile(480, 64 + 192, 96, 96),
				bigHole: mapTile(512, 0 + 192, 64, 64),
				smallSpot: mapTile(480, 0 + 192, 32, 32),
				tinySpot: mapTile(480, 32 + 192, 32, 32),
			},
		},

		sand : {
			light : {
				base : {
					hole : mapTile(0, 448, 32, 32),
					specks : mapTile(32, 448, 32, 32),
					main : mapTile(64, 448, 32, 32),
				},
				patch : mapTile(0, 352, 96, 96),
				bigHole: mapTile(32, 288, 64, 64),
				smallSpot: mapTile(0, 288, 32, 32),
				tinySpot: mapTile(0, 320, 32, 32),
			},
		}

	},
	
	home : {
		floor :{
			img : singleImage(mapAtlasImage),
			panels : [
				 	mapTile(960, 480, 32, 32),
				 	mapTile(992, 480, 32, 32),
				 	mapTile(960, 512, 32, 32),
				 	mapTile(992, 512, 32, 32),
				 	mapTile(960, 546, 32, 32),
				 	mapTile(992, 546, 32, 32),
				]				

		},

		walls : {
			img : singleImage(homeAtlasOne),
			vertical: mapTile(960,586, 32,118),
			horizontal: mapTile(992, 586, 32,86)
		},

		chest :{
			img: singleImage(homeAtlasOne),
			closed : mapTile(480,672, 32,32),
			open : mapTile(480,705, 32,32),

		},

		bed : {
			img : singleImage(homeAtlasTwo),
			vertical: mapTile(8, 942, 48, 81)
		},

		craft : {
			img : singleImage(homeAtlasTwo),
			anvil: mapTile(194, 352, 26, 30),
			stove : mapTile(192, 973, 36, 46),

		},

		table : {
			img : singleImage(homeAtlasTwo),
			one: mapTile(456, 944, 48, 80)
		},

		well : {
			img : singleImage(wellImage),
			arms : {
				one: mapTile(19, 119, 90, 64),
			},

			base : {
				main: mapTile(0, 16, 64, 48),
				frontLayer: mapTile(0,64, 64,32)
			},

			bucket : {
				water: mapTile(69, 0, 22, 63),
				empty : mapTile(101, 0, 22, 63),
			},

			handles : {
				one: mapTile(128, 128, 25, 32),
				two: mapTile(160, 128, 25, 32),
				three: mapTile(192, 128, 25, 32),
			}
		}
	}
}

let itemImages = {
	ore : {
		copper : {
			img : singleImage(oreItems),
			pos : mapTile(0,0, 33, 33) 
		},

		tin : {
			img : singleImage(oreItems),
			pos : mapTile(33,0, 33, 33) 
		},

		iron : {
			img : singleImage(oreItems),
			pos : mapTile(66,0, 33, 33) 
		},

		gold : {
			img : singleImage(oreItems),
			pos : mapTile(99,0, 33, 33) 
		},

		platinum : {
			img : singleImage(oreItems),
			pos : mapTile(0,33, 33, 33) 
		},
		diamond : {
			img : singleImage(oreItems),
			pos : mapTile(0,66, 33, 33) 
		}
	},

	bar : {
		bronze: {
			img : singleImage(barItems),
			pos : mapTile(0,0, 16, 12) 
		},

		iron : {
			img : singleImage(barItems),
			pos : mapTile(10,0, 16, 12) 
		},

		gold : {
			img : singleImage(barItems),
			pos : mapTile(24,0, 16, 12) 
		},

		platinum : {
			img : singleImage(barItems),
			pos : mapTile(42,0, 12, 12) 
		},
		diamond : {
			img : singleImage(barItems),
			pos : mapTile(54,0, 16, 12) 
		}
	},

	wood : {
		oak: {
			img : singleImage(woodItems),
			pos : mapTile(0,0, 21, 30) 
		},

		maple : {
			img : singleImage(woodItems),
			pos : mapTile(21,0, 21, 30) 

		},

		mahogony : {
			img : singleImage(woodItems),
			pos : mapTile(42,0, 21, 30) 

		},

		magic : {
			img : singleImage(woodItems),
			pos : mapTile(63,0, 21, 30) 

		},
		super : {
			img : singleImage(woodItems),
			pos : mapTile(84,0, 21, 30) 

		}
	},

	seed : {
		carrot: {
			img : singleImage(seeds),
			pos : mapTile(32, 0, 32, 32) 
		},

		potato : {
			img : singleImage(seeds),
			pos : mapTile(64, 0, 32, 32) 

		},

		corn : {
			img : singleImage(seeds),
			pos : mapTile(0, 0, 32, 32) 

		},

		cucumber : {
			img : singleImage(seeds),
			pos : mapTile(96, 0, 32, 32) 

		},
		tomato : {
			img : singleImage(seeds),
			pos : mapTile(128, 0, 32, 32) 
		},

		artichoke : {
			img : singleImage(seeds),
			pos : mapTile(128, 0, 32, 32) 

		}
	},

	water : {
		tiny: {
			img : singleImage(potionImages),
			pos : mapTile(24, 72, 24, 24) 
		},

		small : {
			img : singleImage(potionImages),
			pos : mapTile(24, 216, 24, 24) 

		},

		medium : {
			img : singleImage(potionImages),
			pos : mapTile(240, 72, 24, 24) 

		},

		large : {
			img : singleImage(potionImages),
			pos : mapTile(240, 216, 24, 24) 

		},
		giant : {
			img : singleImage(potionImages),
			pos : mapTile(240, 96, 24, 24) 
		},

		god : {
			img : singleImage(potionImages),
			pos : mapTile(240, 240, 24, 24) 

		}
	},


	potion : {
		tiny: {
			img : singleImage(potionImages),
			pos : mapTile(24, 72 - 24, 24, 24) 
		},

		small : {
			img : singleImage(potionImages),
			pos : mapTile(24, 216 - 24, 24, 24) 

		},

		medium : {
			img : singleImage(potionImages),
			pos : mapTile(240, 72 - 24, 24, 24) 

		},

		large : {
			img : singleImage(potionImages),
			pos : mapTile(240, 216 - 24, 24, 24) 

		},
		giant : {
			img : singleImage(potionImages),
			pos : mapTile(240, 96 + 24, 24, 24) 
		},

		god : {
			img : singleImage(potionImages),
			pos : mapTile(240, 240 + 24, 24, 24) 

		}
	},



	plants : {
		carrot: {
			img : singleImage(plantImages),
			pos : mapTile(64, 352, 32, 32) 
		},

		potatoes : {
			img : singleImage(plantImages),
			pos : mapTile(32, 352, 32, 32) 

		},

		corn : {
			img : singleImage(plantImages),
			pos : mapTile(192, 352, 32, 32) 

		},

		cucumber : {
			img : singleImage(plantImages),
			pos : mapTile(160, 352, 32, 32) 

		},
		tomatoes : {
			img : singleImage(plantImages),
			pos : mapTile(0, 352, 32, 32) 
		},

		artichoke : {
			img : singleImage(plantImages),
			pos : mapTile(96, 352, 32, 32) 

		}
	},

	cookedPlants : {
		carrot: {
			img : singleImage(cookedPlantImages),
			pos : mapTile(64, 352, 32, 32) 
		},

		potatoes : {
			img : singleImage(cookedPlantImages),
			pos : mapTile(32, 352, 32, 32) 

		},

		corn : {
			img : singleImage(cookedPlantImages),
			pos : mapTile(192, 352, 32, 32) 

		},

		cucumber : {
			img : singleImage(cookedPlantImages),
			pos : mapTile(160, 352, 32, 32) 

		},
		tomatoes : {
			img : singleImage(cookedPlantImages),
			pos : mapTile(0, 352, 32, 32) 
		},

		artichoke : {
			img : singleImage(cookedPlantImages),
			pos : mapTile(96, 352, 32, 32) 

		}
	},

	cookedMeat : {
		chicken: {
			img : singleImage(cookedFood),
			pos : mapTile(64, 800, 32, 32) 
		},

		cow : {
			img : singleImage(cookedFood),
			pos : mapTile(0, 800, 32, 32) 

		},

		llama : {
			img : singleImage(cookedFood),
			pos : mapTile(32 * 5, 800, 32, 32) 

		},

		pig : {
			img : singleImage(cookedFood),
			pos : mapTile(32 * 12, 800, 32, 32) 

		},
		turkey : {
			img : singleImage(cookedFood),
			pos : mapTile(32 * 3, 800, 32, 32) 
		},

	},

	rawMeat : {
		chicken: {
			img : singleImage(cookedFood),
			pos : mapTile(8*32, 800, 32, 32) 
		},

		cow : {
			img : singleImage(cookedFood),
			pos : mapTile(9*32, 800, 32, 32) 

		},

		llama : {
			img : singleImage(cookedFood),
			pos : mapTile(32 * 10, 800, 32, 32) 

		},

		pig : {
			img : singleImage(cookedFood),
			pos : mapTile(32 * 11, 800, 32, 32) 

		},
		turkey : {
			img : singleImage(cookedFood),
			pos : mapTile(32 * 8, 800, 32, 32) 
		},

	}
}

let magicImages = {
	teleportStone : {
			img : singleImage(magicStones),
			pos : mapTile(264, 66, 64, 64) 
	},

	rangeStone : {
			img : singleImage(magicStones),
			pos : mapTile(199, 1, 64, 64) 
	},

	defenseStone : {
			img : singleImage(magicStones),
			pos : mapTile(134, 1, 64, 64) 
	},

	attackStone : {
			img : singleImage(magicStones),
			pos : mapTile(68, 1, 64, 64) 
	},

	miningStone : {
			img : singleImage(magicStones),
			pos : mapTile(1, 64, 64, 64) 
	},

	thirstStone : {
			img : singleImage(magicStones),
			pos : mapTile(68, 66, 64, 64) 
	},

	hungerStone : {
			img : singleImage(magicStones),
			pos : mapTile(199, 66, 64, 64) 
	},

	woodcutStone : {
			img : singleImage(magicStones),
			pos : mapTile(134, 66, 64, 64) 
	},

	huntingStone : {
			img : singleImage(magicStones),
			pos : mapTile(264, 1, 64, 64) 
	},
}

let userInterface = {
	inventory : {
		img : singleImage(inventoryUI),
		pos : mapTile(0, 0, 120, 480) 
	},

	bankBackGround : {
		img : singleImage(bankBackGround),
		pos : mapTile(0, 0, 300, 430) 
	},

	buyBackGround : {
		img : singleImage(bankBackGround),
		pos : mapTile(0, 0, 300, 430) 
	},

	itemBorder : {
		img : singleImage(itemBorder),
		pos : mapTile(0, 0, 40, 40) 
	},


	armor : {
		img : singleImage(armorUI),
		pos : mapTile(0, 0, 120, 480) 
	},
	stats : {
		img : singleImage(statsUI),
		pos : mapTile(0, 0, 120, 480) 
	},

	redButton : {
		img : singleImage(redButton),
		pos : mapTile(0, 0, 230, 80) 
	},
	greenButton : {
		img : singleImage(greenButton),
		pos : mapTile(0, 0, 230, 80) 
	},
	aquaButton : {
		img : singleImage(aquaButton),
		pos : mapTile(0, 0, 230, 80) 
	},
	
	blueButton : {
		img : singleImage(blueButton),
		pos : mapTile(0, 0, 230, 80) 
	},

	closeButton : {
		img : singleImage(closeButton),
		pos : mapTile(0, 0, 64, 64) 
	},

	downArrow : {
		img : singleImage(downArrow),
		pos : mapTile(0, 0, 64, 64) 
	},

	upArrow : {
		img : singleImage(upArrow),
		pos : mapTile(0, 0, 64, 64) 
	},

	bankScrollBar : {
		img : singleImage(bankScrollBar),
		pos : mapTile(0, 0, 8, 120) 
	},


	statsIcon : {
		img : singleImage(icons),
		pos : mapTile(5, 5, 30, 30) 
	},

	magicIcon : {
		img : singleImage(icons),
		pos : mapTile(5, 40, 30, 30) 
	},

	inventoryIcon : {
		img : singleImage(icons),
		pos : mapTile(40, 40, 30, 30) 
	},

	armorIcon : {
		img : singleImage(icons),
		pos : mapTile(110, 40, 30, 30) 
	},

	actionButtonBackGround : {
		img : singleImage(actionButtonBackGround),
		pos : mapTile(0, 0, 431, 196) 
	},

	itemInfoBackGround : {
		img : singleImage(itemInfoBackGround),
		pos : mapTile(0, 0, 431, 196) 
	},

	healthBar: {
		img : singleImage(healthBar),
		pos : mapTile(0, 0, 206, 28) 
	},

	hungerBar: {
		img : singleImage(hungerBar),
		pos : mapTile(0, 0, 206, 28) 
	},

	thirstBar: {
		img : singleImage(thirstBar),
		pos : mapTile(0, 0, 206, 28) 
	},

	emptyBar: {
		img : singleImage(emptyBar),
		pos : mapTile(0, 0, 206, 28) 
	},


}


export {
	playerImages,
	playerShirts,
	playerPants,
	playerHair,
	playerTools,
	playerWeapons,
	playerArmor,
	enemyImages,
	trees,
	ore,
	npc,
	farmImages,
	farmPlants,
	mapAtlas, 
	itemImages,
	magicImages,
	userInterface

}